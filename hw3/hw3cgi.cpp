    ...
    if (len > 0)
    {
        for(tmp = strtok(buf,"\n"); tmp; tmp = strtok(NULL,"\n"))
        {
            if (contain_prompt(tmp)) 
            {
                ...
                sprintf(line, "%d ", params[userno].line++);
                strcpy(params[userno].buffer, line);
                strcat(params[userno].buffer, tmp);

                printHtml(params[userno].buffer, userno, false);
            }
            else
            {
                ...
                sprintf(line, "%d ", params[userno].line++);
                strcpy(params[userno].buffer, line);
                strcat(params[userno].buffer, lineFeed);
